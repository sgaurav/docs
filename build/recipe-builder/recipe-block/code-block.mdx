...
<Warning> Always connect the If Fails node to a fallback message or error handling block. This ensures the call does not hang or drop abruptly if your script encounters an unexpected issue. </Warning>
...
<Tip> Use the Code Block to sanitize data before passing it to a SmartAI block. For example, converting a raw timestamp (e.g., "1600") into spoken time (e.g., "4 PM") using code is faster and more reliable than asking the AI to figure it out. </Tip>
